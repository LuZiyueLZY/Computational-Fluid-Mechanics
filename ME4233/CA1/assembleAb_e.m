function [A,b]=assembleAb_e(Nx,Ny,dx,dy,x,y,C)
...
for i=2:Nx-2
        ...
        b(po)=-8 * C * pi^2 * cos(2*pi*(2*x(i)+3*y(j))) * dx2 - (dx2/(9*dy2))*C*cos(4*pi*x(i+1));
end
...
for j=2:Ny-2
        ...
        b(po)=-8 * C * pi^2 * cos(2*pi*(2*x(i)+3*y(j))) * dx2- (1/4)*C*cos(6*pi*y(j+1));
end
...
i=1;j=1;
        ...
        b(po)=-8 * C * pi^2 * cos(2*pi*(2*x(i)+3*y(j))) * dx2- (1/4)*C*cos(6*pi*y(j+1))- (dx2/(9*dy2))*C*cos(4*pi*x(i+1));
