...
dt=0.001;   % dt_max = 0.01327 vonneuman analysis
tf=2; % Total time
...
vort = zeros(Nx-1,Ny-1); % b matrix initial guess
t=0;
...
for i=1:NMax
stmfunc = solve_Poisson(vort,A,Nx,Ny); % solving the Poisson equation for streamfunction
                                       
vort = advance_vort(stmfunc,vort,Nx,Ny,dx,dy,dt,Re,f,t); % advance the vorticity transport equation
...
figure(4)
subplot(1,1,1)
plot(dt:dt:dt*i,u_value_for_point_C_array,'-r')
